...
function getAuthEmail(req) {
  ...
  if (req.header('meshblu_auth_email')) {
    authEmail = req.header('meshblu_auth_email');
  }
...
function setupHttpRoutes(server, skynet){
  ...
  function authorizeUserRequest(req, res, next){
    var result = getAuthEmail(req);
    if (!result.email) {
      return res.status(401).send({error: 'unauthorized'});
    }

    knotUser.getUserByEmail(result.email, function (error, user) {
      if(!user){
        if (error) {
          debug('authDevice error', result.email, error.stack);
        }
        error = error || new Error('unauthorized');
        meshbluEventEmitter.emit('identity-error', {error: error.message, request: {email: getAuthEmail(req).email}});
        return res.status(401).send({error: error.message});
      }
      return next(user);
    });
  ...
  server.get('/devices/user', function(req, res){
    authorizeUserRequest(req, res, function(user){
      var data = {user: true};
